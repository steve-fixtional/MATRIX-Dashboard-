...
const regex = /<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">\s*\{files\.map\(\(file\) => \{\s*return \([\s\S]*?\);\s*\}\)\}\s*<\/div>/;

const newGrid = `<VirtuosoGrid
            useWindowScroll
            ...
            itemContent={(index) => {
              ...
              const { icon: TypeIcon } = getFileTypeInfo(file.mimeType, file.name);
              return (
                <div
